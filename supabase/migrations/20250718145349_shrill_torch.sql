...
-- Enable RLS on stripe_customers if not already enabled
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_class WHERE relname = 'stripe_customers' AND relrowsecurity = true
    ) THEN
...
-- Enable RLS on stripe_subscriptions if not already enabled
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_class WHERE relname = 'stripe_subscriptions' AND relrowsecurity = true
    ) THEN
...
-- Enable RLS on stripe_orders if not already enabled
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_class WHERE relname = 'stripe_orders' AND relrowsecurity = true
    ) THEN
...
-- Create or replace view for user subscriptions
